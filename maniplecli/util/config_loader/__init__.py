...
class ConfigLoader():
    ...
    def add_defaults(config):
        if config['lambda_name'] is None:
            click.secho('Please set Lambda name.', fg='red')
            sys.exit(1)
        try:
        ...
        for f in files:
            if config['script'] == None:
                ...
                if f[-2:] == 'py' and 'python' in runtime and handler == f[:-3]:
                    config['script'] = Path(f).resolve().__str__()
            if f == 'requirements.txt' and config['requirements'] is None and 'python' in runtime:
                config['requirements'] = Path(f).resolve().__str__()
            if f == 'package.json' and config['requirements'] is None and 'nodejs' in runtime:
                config['requirements'] = Path(f).resolve().__str__()
            if f == config['tf_file']:
                try:
                    ...
                    if config['s3_key'] == None:
                        config['s3_key'] = ConfigLoader._determine_version(
                            tf['resource']['aws_lambda_function'][config['lambda_name']]['s3_key'],
                            tf)
                except KeyError:
